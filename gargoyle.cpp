...
void gargoyle::fight(const std::vector<std::shared_ptr<players>>* player)
{
...
void gargoyle::damage(double attac)
{
...
void gargoyle::set_stun(bool input) { not_stunned = input; }

//Member function to add effects to the effect list
void gargoyle::add_effects(std::pair<std::string, int> effect) { effects_list.push_back(effect); }
//Member function to handle all the current active effects on the user and complete what they do
void  gargoyle::effects()
{
...
double gargoyle::get_health() { return health; }
std::string gargoyle::get_name() { return name; }
std::vector<std::pair<std::string, int>> gargoyle::show_effects() { return effects_list; }
