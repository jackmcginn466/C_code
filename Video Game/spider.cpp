...
//Definition of member functions for the spider enemy class

spider::spider() {}
spider::~spider() {}

//Member function to complete monsters move during combat
void spider::fight(const std::vector<std::shared_ptr<players>>* player)
{
...
void spider::damage(double attac)
{
...
void spider::set_stun(bool input) { not_stunned = input; }

//Member function to add effects to the effect list
void spider::add_effects(std::pair<std::string, int> effect) { effects_list.push_back(effect); }
...
double spider::get_health() { return health; }
std::string spider::get_name() { return name; }
std::vector<std::pair<std::string, int>> spider::show_effects() { return effects_list; }
